...
export function DiffViewer({ diff, loading, staged, repoPath, onActionComplete }: DiffViewerProps) {
  // ── 所有 hooks 必须在任何 early return 之前定义 ──
  ...
  const isLineSelected = useCallback((hunkIdx: number, lineIdx: number): boolean => {
    try {
      return selectedLines.get(`h${hunkIdx}`)?.has(lineIdx) ?? false;
    } catch { return false; }
  }, [selectedLines]);
    ...
    (hunkIdx: number, lineIdx: number, e: React.MouseEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        setSelectedLines((prev) => {
          ...
          const prevSet = next.get(key);
          if (prevSet) { for (const v of prevSet) set.add(v); }
          next.set(key, set);
          return next;
        });
      } else {
  ...
  const getSelections = useCallback((): SelectionRange[] => {
    try {
      ...
      for (const [key, lines] of selectedLines) {
        ...
        if (!match) continue;
        const hunkIdx = parseInt(match[1], 10);
  ...
  const safeCall = useCallback(async (label: string, fn: () => Promise<void>) => {
    try {
      ...
      try { setActionLoading(false); } catch {}
    }
  }, [clearSelection, onActionComplete]);

  const executeAction = useCallback(
    (type: 'stage' | 'unstage' | 'discard') => {
      ...
      safeCall(type, async () => {
        ...
        if (selections.length > 0) {
          switch (type) {
            case 'stage': await window.electronAPI.workdir.stageLines(repoPath, file, selections); break;
            case 'unstage': await window.electronAPI.workdir.unstageLines(repoPath, file, selections); break;
            case 'discard': await window.electronAPI.workdir.discardLines(repoPath, file, selections); break;
          }
        } else {
          switch (type) {
            case 'stage': await window.electronAPI.workdir.stage(repoPath, [file]); break;
            case 'unstage': await window.electronAPI.workdir.unstage(repoPath, [file]); break;
            case 'discard': await window.electronAPI.workdir.discard(repoPath, [file]); break;
          }
        }
      });
    },
  ...
  const executeHunkAction = useCallback(
    (type: string, hunkIdx: number) => {
      if (!repoPath || !diff) return;
      safeCall(type, async () => {
        const file = diff.file;
        switch (type) {
          case 'stageHunk': await window.electronAPI.workdir.stageHunk(repoPath, file, hunkIdx); break;
          case 'unstageHunk': await window.electronAPI.workdir.unstageHunk(repoPath, file, hunkIdx); break;
          case 'discardHunk':
            await window.electronAPI.workdir.discardLines(repoPath, file, [
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2 bg-gray-800 border-b border-gray-700 flex-shrink-0">
        ...
        {isStaged && (
          <span className="text-xs bg-blue-900/40 text-blue-300 px-1.5 py-0.5 rounded">已暂存</span>
        )}
        <div className="ml-auto flex items-center gap-1">
          {hasSelection && (
            <span className="text-xs text-gray-500 mr-1">已选 {totalSelected} 行</span>
          )}
          {isStaged ? (
            <button onClick={() => executeAction('unstage')} disabled={actionLoading}
              className="flex items-center gap-1 px-3 py-1 text-xs bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 disabled:text-gray-500 rounded transition-colors">
              <Minus className="w-3.5 h-3.5" />{hasSelection ? '取消暂存选中行' : '取消暂存全部'}
            </button>
          ) : (
            <>
              <button onClick={() => executeAction('stage')} disabled={actionLoading}
              ...
              <button onClick={() => executeAction('discard')} disabled={actionLoading}
          ...
          {hasSelection && (
            <button onClick={clearSelection} className="px-2 py-1 text-xs text-gray-400 hover:bg-gray-700 rounded transition-colors">清除选择</button>
          )}
        </div>
      </div>
            ...
            <div key={hunkIdx}>
              <div onClick={() => toggleHunk(hunkIdx)}
                ...
                <div className="flex items-center gap-0.5 ml-2 opacity-0 hover:opacity-100 transition-opacity">
                  {isStaged ? (
                    <button onClick={(e) => { e.stopPropagation(); executeHunkAction('unstageHunk', hunkIdx); }}
                      className="p-1 hover:bg-orange-900/30 rounded text-orange-400" title="取消暂存此块">
                      <Minus className="w-3 h-3" />
                    </button>
                  ) : (
                    <>
                      <button onClick={(e) => { e.stopPropagation(); executeHunkAction('stageHunk', hunkIdx); }}
                        className="p-1 hover:bg-green-900/30 rounded text-green-400" title="暂存此块">
                        <Plus className="w-3 h-3" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); executeHunkAction('discardHunk', hunkIdx); }}
              ...
              {!collapsedHunks.has(hunkIdx) && (
                <div className="relative">
                  ...
                  {hunk.lines.map((line, lineIdx) => {
                    ...
                    return (
                      <div key={lineIdx}
                        onClick={(e) => handleLineClick(hunkIdx, lineIdx, e)}
                        ...
                        <div className="flex items-center mr-1">
                          <div className={`w-3 h-3 rounded border transition-colors flex items-center justify-center ${
                            selected ? 'bg-blue-500 border-blue-500' : 'border-gray-600 opacity-0 group-hover:opacity-100'
                          }`}>
                            {selected && (
