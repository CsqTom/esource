...
import {
  ChevronDown, ChevronRight, Minus, FileCode, MousePointer2, AlertCircle,
} from 'lucide-react';

interface DiffStageViewProps {
  diff: SerializedDiff;
  loading?: boolean;
  repoPath?: string;
...
/** 已暂存文件的差异查看器，仅支持"取消暂存"操作 */
export function DiffStageView({ diff, loading, repoPath, onActionComplete }: DiffStageViewProps) {
  ...
  const handleUnstage = useCallback(() => {
    ...
    safeCall('unstage', async () => {
      const selections = getSelections(); const file = diff.file;
      if (selections.length > 0) {
        await window.electronAPI.workdir.unstageLines(repoPath, file, selections);
      } else {
        await window.electronAPI.workdir.unstage(repoPath, [file]);
      }
    });
  }, [repoPath, diff, getSelections, safeCall]);

  const handleUnstageHunk = useCallback((hunkIdx: number) => {
    if (!repoPath || !diff) return;
    safeCall('unstageHunk', async () => {
      await window.electronAPI.workdir.unstageHunk(repoPath, diff.file, hunkIdx);
    });
  ...
  if (!diff || !diff.hunks || diff.hunks.length === 0) return (<div className="flex-1 flex items-center justify-center text-gray-500"><div className="text-center"><FileCode className="w-12 h-12 mx-auto mb-2 opacity-50" /><p>没有差异内容</p></div></div>);
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2 bg-gray-800 border-b border-gray-700 flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <button onClick={handleUnstage} disabled={actionLoading}
            ...
            <div key={hunkIdx}>
              <div onClick={() => toggleHunk(hunkIdx)}
                ...
                <div className="flex items-center gap-0.5 ml-2 opacity-0 hover:opacity-100 transition-opacity">
                  <button onClick={(e) => { e.stopPropagation(); handleUnstageHunk(hunkIdx); }}
